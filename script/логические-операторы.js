...
function typeAll () {
  const check1 = typeCountry.includes('рф') || typeCountry.includes('россия') ? true : alert('Только гражданам РФ');
  const check2 = typeAge >= 18 ? true : alert('Выдаем кредит только совершеннолетним');
  const check3 = typeDebt <= 0 ? true : alert("У вас не должно быть долгов");

  const checkLast = check1 && check2 && check3 ? 'Вам одобрен кредит' : 'Вам отказано!';
  console.log(alert(checkLast));
}
